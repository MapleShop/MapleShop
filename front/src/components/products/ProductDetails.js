...
import MetaData from "../layout/MetaData"
import {useDispatch, useSelector} from 'react-redux'
import { useParams} from 'react-router-dom' //Necesito importar esta herramienta para poder declarar el ID
...
export const ProductDetails = () => {
  ...
  const {id} =useParams(); //Declaro el Id, useParams busca algo que se llama Id y lo guarda en id
...
=======
  useEffect(()=>{
    dispatch(getProductDetails(id)) //Desde donde estamos esperando un despacho o un envío, desde un getProductDetails desde nuestros Actions
    if (error){
      alert.error(error);
      dispatch(clearErrors())
    }
  },[dispatch, alert, error, id]) //Ahora le pedimos que nos muestre lo que despacho
>>>>>>> 82ead59e64177e7ff1c2ba4fb49253be586e69b3
...
<<<<<<< HEAD
            ...
            <div className='col-12 col-lg-5 mt-5'>
                ...
                <spin id="No_de_reviews">  ({product.numCalificaciones} Reviews)</spin>
                <hr />
...
=======
            ...
            <div className='col-12 col-lg-5 mt-5'>
                ...
                <button type="button" id="carrito_btn" className="btn btn-primary d-inline ml-4" sisabled={product.inventario===0}>Agregar al Carrito</button>
                <hr />
                <p>Estado: <span id="stock_stado" className={product.inventario>0 ? 'greenColor':'redcolor'}>{product.inventario>0 ? "En existencia":"Agotado"}</span></p>
                <hr />
